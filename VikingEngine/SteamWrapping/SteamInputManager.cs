...
namespace VikingEngine.SteamWrapping
{
        ...
        SteamInputManager
    {
        ...
        Callback<GamepadTextInputDismissed_t> _textInputDismissedCallback;
        ITextInputReciever textInputReciever;
        ...
        public SteamInputManager()
        {
            ...
            _textInputDismissedCallback = Callback<GamepadTextInputDismissed_t>.Create(OnTextInputDismissed);
        }
        ...
        public static bool tryOpenSteamKeyboard(ITextInputReciever reciever)
        {
            return Ref.steam.isInitialized && Ref.steam.input.OpenSteamKeyboard(reciever);
        }

        /// <summary>
        /// Only works in big picture mode
        /// </summary>
        public bool OpenSteamKeyboard(ITextInputReciever reciever)
        {
            // Make sure the overlay is actually enabled/working
            if (SteamUtils.IsOverlayEnabled())
            {
                textInputReciever = reciever;
                // Parameters: Input Mode, Line Mode, Description, Max Characters, Existing Text
                bool success = SteamUtils.ShowGamepadTextInput(
                    ...
                    "Type here",
                    40,
                    reciever.textInput_CurrentText() // Pre-fill with their current name
                );
        ...
        // Handle steam keyboard result
        private void OnTextInputDismissed(GamepadTextInputDismissed_t callback)
        {
            
            if (callback.m_bSubmitted)// pressed "Submit"
            {
                // Get the length of what they typed
                uint length = SteamUtils.GetEnteredGamepadTextLength();

                // Retrieve the actual text
                string enteredText;
                if (SteamUtils.GetEnteredGamepadTextInput(out enteredText, length))
                {
                    //_playerName = enteredText;
                    //Console.WriteLine("Player entered: " + enteredText);
                    textInputReciever.SteamInputExit(enteredText);//textInput_complete(enteredText, textInputReciever.textInput_Tag());
                    return;
                }
            }

            textInputReciever.SteamInputExit(null);
            //else //"Cancel/B"
            //{
            //    textInputReciever.textInput_complete(null, textInputReciever.textInput_Tag());
            //    //Console.WriteLine("Player canceled text input.");
            //}
        }
