                    ...
                    if (bufferGoal_percentage > 0)
                    {
                        ...
                        void orderMissingFood(float hasAmount, float goalAmount, int cityAmount, int goldValue, ItemResourceType foodType)
                        {
                                ...
                                faction.payGold(goldValue, false, this))
                            {
                                ...
                                status.createWorkOrder(WorkType.TrossCityTrade, (int)foodType, 0, XP.WorkExperienceType.NUM_NONE, WorkPriorityType.NUM_NONE, -1, WP.ToSubTilePos_Centered(city.tilePos), null);
                                if (goldValue > 0)
                                {
