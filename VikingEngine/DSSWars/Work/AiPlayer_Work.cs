...
namespace VikingEngine.DSSWars.Players
{
    partial class AiPlayer
    {
        ...
        public override void AutoExpandType(City city, out bool work, out Build.BuildAndExpandType building, out bool intelligent)
        {
            ...
            if (city.needMore(CityResourceIndex.rawFood) && Ref.peRnd.Chance(0.6))
            {
                building = BuildAndExpandType.WheatFarm;
            }
            else if (city.resourceAmount(CityResourceIndex.fuel) < ResourceLowBuffer && city.resourceAmount(CityResourceIndex.wood)/*res_wood.amount*/ > ResourceLowBuffer && Ref.rnd.Chance(0.6))
            {
                building = BuildAndExpandType.CoalPit;
            }
            else if (city.needMore(CityResourceIndex.skinLinnen) && Ref.peRnd.Chance(0.6))
            {
            ...
            {
                var res_ironore = city.GetGroupedResource(CityResourceIndex.ironore);
