...
namespace VikingEngine.DSSWars.Map
{
    class UnitCollAreaGrid
    {
                ...
                {  
                    if (grid.TryGet(x, y, out area))
                    {
                        lock (area.armies)
                        {
                            ...
                            {
                                if (RefExt.EqTarget(faction, obj.faction)) //obj.faction == faction)
                                {
                ...
                for (int x = areaPos.X - 1; x <= areaPos.X + 1; ++x)
                {
                    if (grid.TryGet(x, y, out area))
                    {
                        ...
                        lock (area.groups)
                        {
                            ...
                            { 
                                if (group.attackTarget_soldierGroupOrCity != null && group.army.faction.TryGetTarget(out var tArmyFaction) && tArmyFaction == player)
                                {
                                    return true;
                                }
                            }
                    ...
                    if (grid.TryGet(x, y, out area))
                    {
                        ...
                        lock (area.groups)
                        {
                            if (area.groups != null)
                            {
                                for (int i = 0; i < area.groups.Count; ++i)
                                {
                                    if (RefExt.EqTarget(faction, area.groups[i].army.faction) &&
        ...
        {
            if (city.faction.TryGetTarget(out var tCityFaction))
            {
                            ...
                            lock (area.groups)
                            {
                                if (area.groups != null)
                                {
                                    foreach (var m in area.groups)
                                    {
                                        ...
                                        {
                                            if (RefExt.EqTarget(m.army.faction, city.faction, out var tArmyFaction)/*city.faction == m.army.faction*/ ||
                                                DssRef.diplomacy.InWar(city.faction, m.army.faction))
                                            {
                                                if (faction_power.TryGetValue(tArmyFaction.parentArrayIndex, out float strength))
                                                {
                                                    faction_power[tArmyFaction.parentArrayIndex] = strength + m.strengthValue();
                                                }
                                                else
                                                {
                                                    faction_power.Add(tArmyFaction.parentArrayIndex, m.strengthValue());
                                                }
                                            }
                                        }
                                    }
                ...
                int strongest = tCityFaction.parentArrayIndex;
        ...
        public void collectOpponentGroups(WeakReference<Faction> faction, IntVector2 tilePos, out List<GameObject.SoldierGroup> groups, out List<City> cities)
        {
                    ...
                    if (grid.TryGet(x, y, out area))
                    {
                        lock (area.armies)
                        {
                            if (area.armies != null)
                            {
                                foreach (var m in area.armies)
                                {
                                    if (RefExt.EqTarget(factionFilter, m.faction))
                                    {
                                        if (!armies.Contains(m))
                                        {
                                            armies.Add(m);
                    ...
                    if (grid.TryGet(x, y, out area))
                    {
                        lock (area.groups)
                        {
                            if (area.groups != null)
                            {
                                foreach (var m in area.groups)
                                {
                                    if (!RefExt.EqTarget(faction,  m.army.faction) &&
                                        m.army != prevArmy)
                                    {
                                        prevArmy = m.GetArmy();
                        ...
                        lock (area.armies)
                        {
                            ...
                            if (area.armies != null)
                            {
                                for (int i = 0; i < area.armies.Count; ++i)
                                {
                                    ...
                                    if (army != ignore &&
                                        RefExt.EqTarget(factionFilter, army.faction) &&
                ...
                if (grid.TryGet(pos, out area))
                {
                    for (int i = 0; i < area.cities.Count; ++i)//foreach (var cityIx in area.cities)
                    {
                        ...
                        if (city.faction.TryGetTarget(out var tCityFaction))
                        {
                            if (factionFilter != null)
                            {
                                if (tCityFaction == factionFilter)
                                {
                                    nearCities.Add(city);
                                }
                            }
                            else if (myFaction != tCityFaction)
                            {
                                nearCities.Add(city);
                            }
                        }
                    ...
                    lock (area.armies)
                    {
                        if (area.armies != null)
                        {
                            foreach (var army in area.armies)
                            {
                                ...
                                {
                                    if (army.faction.TryGetTarget(out var tArmyFaction))
                                    {
                                        if (factionFilter != null)
                                        {
                                            if (tArmyFaction == factionFilter)
                                            {
                                                nearMapObjects.Add(army);
                                            }
                                        }
                                        else if (myFaction != tArmyFaction)
                                        {
                                            nearMapObjects.Add(army);
                                        }
                                    }
    ...
    struct ArmyId
    {
        ...
        {
            if (army.faction.TryGetTarget(out var tFaction))
            {
                faction = tFaction.parentArrayIndex;
            }
            this.army = army.parentArrayIndex;
        }
