...
namespace VikingEngine.DSSWars.GameObject
{
    ...
    partial class SoldierGroup : AbsGroup
    {
        ...
        Vector3 walkingGoalAttackTarget(AbsGroup target, out bool shiptransform)
        {
            ...
            return target.position;//flankTargetPositon(target);
        }
        ...
        //Vector3 flankTargetPositon(AbsGroup target)
        //{
        //    Vector3 result = target.position;

        //    //if (armyGridPlacement2.X != 0)
        //    //{
        //    //    Vector2 dir = VectorExt.Normalize(
        //    //        target.position.X - position.X,
        //    //        target.position.Z - position.Z, out float l);
        ...
        //    //        result.X += dir.X * FlankDistAdd;
        //    //        result.Z += dir.Y * FlankDistAdd;
        ...
        public void asynchNearObjectsUpdate()
        {
            ...
            if (nearest != null)
            {
                ...
                if (!nearest.defeatedBy(army.faction) && nearest != attackTarget_soldierGroupOrCity)
                {
                    ...
                    {
                        //Compare distance
                        if (distanceValueTo(attackTarget_soldierGroupOrCity, float.MaxValue) * 2f <= distanceValueTo(nearest, float.MaxValue))
                        {
                            return;
                        }
                    }
                    ...
                    attackTarget_soldierGroupOrCity = nearest;
                }
            }
        ...
        float distanceValueTo(AbsGroup toGroup, float maxRange)
        {
            ...
            if (toGroup == attackTarget_soldierGroupOrCity)
            {
                value *= 0.5f;
            }
        ...
        //{
            ...
            if (attack_sp != null)
            {
                if (soldiers != null)
                {
                    ...
                    IntVector2 goalSubTile = WP.ToSubTilePos(attack_sp.position);//flankTargetPositon(attack_sp));
                    var detailPath_sp = detailPath;
        ...
        public override void stateDebugText(RichBoxContent content)
        {
            ...
            if (attackTarget_soldierGroupOrCity!=null)
            {
                ...
                //while (c.Next())
                //{
                    content.text("attacking: " + attackTarget_soldierGroupOrCity.TypeName());
                //}
            }
            else
            {
