            ...
            else if (autoBuild_Farm)
            {
                ...
                switch (autoExpandFarmType)
                {
                    case BuildAndExpandType.OrchardApple:
                        needMore = GetGroupedResource(CityResoureIndex.food).needMore();
                        break;
                    case BuildAndExpandType.WheatFarm:
                        needMore = GetGroupedResource(CityResoureIndex.rawFood).needMore();
                        break;
                    case BuildAndExpandType.LinenFarm:
                        needMore = GetGroupedResource(CityResoureIndex.skinLinnen).needMore();
                        break;
                    case BuildAndExpandType.RapeSeedFarm:
                        needMore = GetGroupedResource(CityResoureIndex.fuel).needMore();
                        break;
                    case BuildAndExpandType.HempFarm:
                        needMore = GetGroupedResource(CityResoureIndex.skinLinnen).needMore() || 
                            GetGroupedResource(CityResoureIndex.fuel).needMore();
                        break;
                    default:
#if DEBUG
                        throw new Exception();
