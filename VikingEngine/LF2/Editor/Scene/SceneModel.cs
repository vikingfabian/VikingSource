...
namespace VikingEngine.LF2.Editor
{
    class SceneModel 
    {
        ...
        public SceneModel(SceneModel original, Scene.SceneCollection gamestate)
        {
            ...
            point = new Graphics.Point3D(original.Model.position, original.Model.scale);
        ...
        {
            DataStream.FilePath path = VoxelDesigner.CustomVoxelObjPath(Name);
            path.Storage = fromStorage;
            if (!fromStorage)
            {
                path.DirectoryPath = LootfestLib.SceneModelFolder;//Scene.SceneCollection.ContentPath +"\\Models";
            }
    ...
    class SceneModelLoader: DataLib.SaveStreamWithQuedProcess
    {
        ...
        {
            Ref.draw.AddToRenderList(model, true);
