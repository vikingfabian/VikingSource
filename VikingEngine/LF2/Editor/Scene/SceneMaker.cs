...
using VikingEngine.LF2.Editor.Scene;
...
namespace VikingEngine.LF2.Editor
{
    ...
    class SceneMaker : Engine.GameState, SceneModelsParent
    {
            ...
            : base()
        {
            ...
            freePencil = new Graphics.Mesh(LoadedMesh.sphere, Vector3.Zero, new Graphics.TextureEffect(
                Graphics.TextureEffectType.LambertFixed, SpriteName.WhiteArea), 0.4f);
            freePencil.Color = Color.Blue;

            selectionBox = new Graphics.Mesh(LoadedMesh.cube_repeating, Vector3.Zero, new Graphics.TextureEffect(
                Graphics.TextureEffectType.Flat, SpriteName.LFMultiSelection), 0f);

            centerMark = new Graphics.Mesh(LoadedMesh.plane, Vector3.Zero, new Graphics.TextureEffect(
                Graphics.TextureEffectType.Flat, SpriteName.WhiteArea), 0.2f);
            centerMark.Color = Color.DarkGray;

            infoText = new Graphics.TextG(LoadedFont.TimesS, Engine.Screen.SafeArea.BottomLeft, Vector2.One * 0.6f, new Graphics.Align(Facing8Dir.SW),
            ...
            DataLib.SaveLoad.CreateFolder(SceneCollection.Folder);
        }
        ...
        {
            collection.randomFileName();
            Engine.XGuide.BeginKeyBoardInput(new KeyboardInputValues("Name", "Name your creation", collection.currentFileName, controller.Index));
        }
        ...
        void moveFreePencil(Vector3 dir)
        {
            if (selectedMember == null || editType == EditType.Move)
            {
                ...
                string info = "pos" + freePencil.Position.ToString();
                //selectClosetObj
                if (collection.members.Count > 0)
                {
                    LowestValue shortestDist = new LowestValue(true);
                    for (int i = 0; i < collection.members.Count; i++)
                    {
                        shortestDist.Next((collection.members[i].Model.position - freePencil.Position).Length(), i);
                    }
                    selectedMemberIndex = shortestDist.LowestMemberIndex;
                    //place box around it
                ...
                camera.Target = freePencil.Position;
        ...
        void updateInput()
        {
            ...
            else if (InMenu)
            {
                ...
                else if (controller.KeyDownEvent(Buttons.B)
                    ...
                    )
                {
                    if (menu.InSubMenu)
                    {
                        menu.Back((int)controller.Index);
                    }
                    else
                        CloseMenu();
                }
                else if (controller.KeyDownEvent(Buttons.Start))
                {
                    CloseMenu();
                }
            }
            else
            {
                ...
                else if (controller.KeyDownEvent(Buttons.A)
                ...
                else if (controller.KeyDownEvent(Buttons.X)
                ...
                else if (controller.KeyDownEvent(Buttons.LeftShoulder))
                {
                    nextEditMode(-1);
                }
            ...
            {
                //sticks
                menu.MoveSelection(controller.JoyStickValue(Stick.Left));
            }
            else
            {
                if (cameraCtlrKeyDown)
                {
                    rotateCamera(controller.JoyStickValue(Stick.Right).DirAndTime);
                    zoom(controller.JoyStickValue(Stick.Left).DirAndTime.Y);
                }
                else
                {
                    const float MouseSpeed = 2.4f;
                    Vector2 rightStick = controller.JoyStickValue(Stick.Right).DirAndTime;
                    moveAction(controller.JoyStickValue(Stick.Left).DirAndTime * MouseSpeed, -rightStick.Y * MouseSpeed);
                    rightStick.Y = 0;
            ...
            if (PlatformSettings.PC_platform)
            {
                ...
                if (!InMenu)
                {
                    ...
                    {
                        nextEditMode(lib.FloatToDir(Input.Mouse.ScrollValue));
                    }
        ...
        {
            camera.Zoom = Bound.Set(camera.Zoom + value * ZoomSpeed, ZoomBounds.Min, ZoomBounds.Max); 
        }
        ...
        {
            editType = (EditType)lib.SetBoundsRollover((int)editType + dir, 0, (int)EditType.NUM - 1);
