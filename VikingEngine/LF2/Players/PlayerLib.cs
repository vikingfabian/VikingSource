...
namespace VikingEngine.LF2.Players
{
    ...
    class GamerName
    {
        ...
        public GamerName(GameObjects.Characters.Hero hero, ClientPlayer owner)
        {
            //Keep = true;
            this.Hero = hero;
            Color col = Color.White;
            if (owner.Name == "GamefarmContact")
            {
                col = Color.Red;
            }
            else if (Playtester(owner.Name))
            {
                col = Color.Yellow;
            }

            text = new TextG(LoadedFont.Lootfest, Vector2.Zero, new Vector2(0.8f), Align.CenterAll, owner.Name, col, ImageLayers.Background3);
        }
    ...
    enum Link
    {
        ...
        FPviewHoriSpeed, FPviewVertiSpeed,
        RemoveDoor,
        NameThisLocation,
        UseHorseTravelOK,
        JoinRaceFromMessage,
