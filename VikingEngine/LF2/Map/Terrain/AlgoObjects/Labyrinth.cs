...
namespace VikingEngine.LF2.Map.Terrain.AlgoObjects
{
    class Labyrinth
    {
        ...
        {
            //pos += IntVector2.NegativeOne + IntVector2.FromDir4(dir);
        ...
        {
            ForXYLoop loop = new ForXYLoop(area.Position, area.BottomRight - 1);
            while (!loop.Done)
            {
        ...
        public void Generate(IntVector2 startPos)
        {
            ...
            while (!done)//tills den gått hela varvet runt och ingen är disconnected
            {
                ...
                {
                    Dir4 dir = (Dir4)Data.RandomSeed.Instance.Next((int)Dir4.NUM_NON);
                    for (int i = 0; i < 4; i++)
                    {
                        dir++;
                        if (dir == Dir4.NUM_NON)
        ...
        IntVector2 rndStartCheck()
        {
            return new IntVector2( Data.RandomSeed.Instance.Next(size.X), Data.RandomSeed.Instance.Next(size.Y));
        }
