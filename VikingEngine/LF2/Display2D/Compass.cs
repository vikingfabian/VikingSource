...
namespace VikingEngine.LF2
{
    class Compass
    {
        ...
        public void Update(PlayState gamestate)
        {
            ...
            compassRot.Radians = MathHelper.TwoPi - (player.localPData.view.Camera.TiltX - MathHelper.PiOver2);
            
            compass.Rotation = compassRot.Radians;
            compassNsymbol.Position = compass.Position + compassRot.Direction(CompassIconPos);
            compassPlayerDir.Rotation = player.hero.FireDir.Radians + compassRot.Radians;
            ...
            {//find compass icons
                ...
                //    ic.removeFlag = true;
                //}
                if (lib.VectorSideLength(player.hero.PlanePos - previousCompassLocation) >= 32)
                {
                    previousCompassLocation = player.hero.PlanePos;

                    clearIcons();

                    //find nearby craftsmen
                    ISpottedArrayCounter<GameObjects.AbsUpdateObj> gameObjects = gamestate.GameObjCollection.AllMembersUpdateCounter;

                    while (gameObjects.Next())
                    {
                        if (gameObjects.GetMember is GameObjects.NPC.AbsNPC)
                        {
                            GameObjects.NPC.AbsNPC npc = gameObjects.GetMember as GameObjects.NPC.AbsNPC;
                            if (npc.CompassIcon != SpriteName.NO_IMAGE)
                            {
                                if (player.hero.distanceToObject(npc) <= CompassIconCraftsMan.MaxDistance)
                                {
