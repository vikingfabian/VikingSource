...
namespace VikingEngine.LF2.GameObjects.Magic
{
    class Lightning : AbsUpdateObj
    {
        ...
        GameObjects.WeaponAttack.AbsWeapon weapon;

        public Lightning(AbsUpdateObj strikeCenter,  GameObjects.WeaponAttack.AbsWeapon weapon)
            : base()
        {
            //damage.Damage *= 0.4f;
            ...
            while(args.allMembersCounter.Next())
            {
                    ...
                    args.allMembersCounter.GetMember != strikeCenter)
                {
                    ...
                    if (length <= AttackLength)
                    {
                        ...
                        numHits++;
                        if (numHits >= LootfestLib.MaxLightningChains)
                            break;
                    }
                }
            }
        ...
        static void createImages(List<AbsUpdateObj> objectOrder, bool local)
        {
            ...
            {
                WeaponAttack.DamageData damage = new WeaponAttack.DamageData(LootfestLib.MaxLightningChains, 
                    objectOrder[0].WeaponTargetType, objectOrder[0].ObjOwnerAndId,
                    Gadgets.GoodsType.NONE, MagicElement.Lightning, WeaponAttack.SpecialDamage.NONE, true);
                ...
                for (int i = 1; i < objectOrder.Count; i++)
                {
                    ...
                    Graphics.AbsVoxelObj lightImg = LF2.Data.ImageAutoLoad.AutoLoadImgInstace(VoxelModelName.lightning, TempImage, 0, 0);//LootfestLib.Images.StandardObjInstance(VoxelModelName.lightning);
                ...
                if (local)
                {
                    ...
                    {
                        obj.ObjOwnerAndId.WriteStream(w);
                    }
                }
        ...
        public static void NetworkReadLighting(System.IO.BinaryReader r, Director.GameObjCollection objColl)
        {
            ...
            {
                AbsUpdateObj obj = objColl.GetActiveOrClientObjFromIndex(r);
                if (obj != null)
                {
                    objectOrder.Add(obj);
                }
