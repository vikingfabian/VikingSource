...
namespace VikingEngine.LF2.GameObjects.EnvironmentObj
{
    ...
    class Door : MapChunkObject, IInteractionObj, IUpdateable
    {
           ...
           // : base(size)
        {
            ...
            for (pos.Y = 0; pos.Y <= data.Limits.Y; pos.Y++)
            {
                for (pos.Z = 0; pos.Z <= data.Limits.Z; pos.Z++)
                {
                    ...
                    {
                        data.MaterialGrid[pos.X, pos.Y, pos.Z] = LfRef.chunks.Get(wp.GetNeighborPos(pos));
                    }
                }
            }
        ...
        public bool CompleteDoor(int pIx)
        {
            ...
            if (differentStates())
            {
                ...
                System.IO.BinaryWriter w = Ref.netSession.BeginWritingPacket(
                    Network.PacketType.LF2_createDoor, Network.PacketReliability.Reliable, pIx);
        ...
        public void InteractEvent(Characters.Hero hero, bool start)
        {
            if (start)
            {
                ...
                {
                    System.IO.BinaryWriter w = Ref.netSession.BeginWritingPacket(
                        Network.PacketType.LF2_OpenCloseDoor, Network.PacketReliability.Reliable, hero.Player.Index);
                    wp.WriteChunkGrindex(w);
        ...
        void openClose()
        {
            ...
            Music.SoundManager.PlaySound(LoadedSound.door, wp.ToV3());
        }

        void createBound(Map.WorldPosition pos)
        {
            Vector3 size = (data.Limits + 1).Vec;
            bound = new VectorVolume(pos.ToV3() + size * PublicConstants.Half, (data.Limits + 6).Vec);
        }

        public bool InRange(Characters.Hero hero)
        {
        ...
        public bool Interact_LinkClick(HUD.IMenuLink link, Characters.Hero hero, HUD.AbsMenu doc) { throw new NotImplementedException(); }
        public HUD.DialogueData Interact_OpeningPhrase(Characters.Hero hero) { throw new NotImplementedException(); }
        public File Interact_TalkMenu(Characters.Hero hero) { throw new NotImplementedException(); }
