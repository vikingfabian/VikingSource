...
{
    class HealUp : AbsInGameUpdateable
    {
        ...
        public HealUp(GO.AbsVoxelObj obj, bool heal_notMagic, bool smallBoost)
            :base(true)
        {
        ...
        //    Data.IReplacementImage replacementImg = heal_notMagic ? HeartTempImage : WaterDropTempImage;
            ...
            for (int i = 0;  i < num; i++)
            {
                symbols[i] = LfRef.modelLoad.AutoLoadModelInstance(model, ScaleRange.GetRandom(), 0, false); 
                symbols[i].position = Ref.rnd.Vector3_Sq(center, 1.2f);
                symbols[i].Rotation.RotateWorld(Vector3.UnitX * Ref.rnd.Rotation()); 
            }
        }
