...
using VikingEngine.LootFest.Map.Terrain.AlgoObjects;
...
#region FILL
        ...
        public void FillRoof(Data.MaterialType material, float depthPercentage, Dir4 ridgeLongitudinalForward, float ridgeLateralOffsetPercentage)
        {
            ...
            for (pos.Z = start.Z; pos.Z < end.Z; ++pos.Z)
            {
                for (pos.Y = start.Y; pos.Y < end.Y; ++pos.Y)
                {
                    for (pos.X = start.X; pos.X < end.X; ++pos.X)
                    {
                        ...
                        float localX = pos.GetDimensionXZ(perpendicular) - leftEnd;
            ...
            for (pos.Z = n.Position.WorldGrindex.Z; pos.Z < end.Z; ++pos.Z)
            {
                for (pos.Y = n.Position.WorldGrindex.Y; pos.Y < n.Position.WorldGrindex.Y + bottomOffset; ++pos.Y)
                {
                    ...
                    {
                        //LfRef.chunks.Set(pos, (byte)Data.MaterialType.RGB_Blue);
                    }
                }
            }
        }

        public void Fill(ushort material)
        {
        ...
        public void FillWalls(ushort material, int thickness)
        {
        ...
        public void FillCylinder(ushort material, Dir4 longitudinalDir, CubeFace cutAway, float radiusMultiplier)// = 1.0f)
        {
            FillCylinder(material, lib.FacingToCubeface(longitudinalDir), cutAway, radiusMultiplier);
        }
        /// <param name="radiusMultiplier">Adjust which blocks that are in radius of center, recommended: 1.1f</param>
        public void FillCylinder(ushort material, CubeFace longitudinalDir, CubeFace cutAway, float radiusMultiplier)// = 1.0f)
        {
            //byte m = (byte)material;
            WorldVolume n = this;
            n.SetYLimit();
                ...
                for (pos.Y = start.Y; pos.Y < end.Y; ++pos.Y)
                {
                    ...
                    for (pos.Z = start.Z; pos.Z < end.Z; ++pos.Z)
                    {
                        ...
                        if (posDiff.Length() <= radiusMultiplier)
                        {
                            ...
                            for (pos.X = start.X; pos.X < end.X; ++pos.X)
                            {
                                pos.SetBlock(material);//LfRef.chunks.Set(pos, m);
                            }
                        }
                    }
                }
        ...
        public void FillSphere(ushort material, float radiusMultiplier)// = 1.0f)
        {
            //byte m = (byte)material;
            WorldVolume n = this;
            n.SetYLimit();
        ...
        public void FillArch(Dir4 facing, ushort material)
        {
        ...
        public void CreateArchway(Dir4 direction, ushort material, int archWidth, int overhang)
        {
            WorldVolume copy = this;
            // Create archway.
            ...
            copy.FillArch(direction, VikingEngine.LootFest.Map.HDvoxel.BlockHD.EmptyBlock);
        }
        ...
        public void FillPillar(ushort pillarMaterial, ushort decorationMaterial,
...
#region TRANSFORM
        ...
        //            AddPosition(lib.GetOppositeDirection(facing), Size.GetDimension(dim));
        //            break;
        //        default:
        ...
        ///// <param name="relPlace">relative placement of the volume</param>
        ///// <param name="wp">the reference position</param>
        //public WorldVolume AlignTo(Facing4Dir facing, RelativePlacement relPlace, WorldPosition wp)
        //{
        ...
        public WorldVolume SubFromLateralEnds(Dir4 dir, int amount)
        {
            ...
            Dir4 perpOpposite = lib.OppositeDir(perpendicular);
        ...
        public WorldVolume SubFromLongitudinalEnds(Dir4 dir, int amount)
        {
            ...
            Dir4 opposite = lib.OppositeDir(dir);
        ...
        public WorldVolume SetLateralWidthCentered(Dir4 dir, int targetWidth)
        {
            ...
            if (diff > 0)
            {
                ...
                AddToSide(lib.OppositeDir(lib.GetPositivePerpendicularFacing(dir)), diff / 2 + diff % 2);
            }
            else
            {
                diff *= -1;
                SubFromSide(lib.GetPositivePerpendicularFacing(dir), diff / 2 + diff % 2);
                SubFromSide(lib.OppositeDir(lib.GetPositivePerpendicularFacing(dir)), diff / 2);
            }
            return this;
        }
...
#region Bottom, Top, Left, Right, Front, Back, Centers, Width, Depth and Height in XZ Airplane mode (Facing4Dir)
        // Only implement all in this region with Facing4Dir, not CubeFace, because
        ...
        {
            return Size.GetDimensionXZ(lib.GetPerpendicularDirection(facing));
        }
        public int GetLongitudinalDepth(Dir4 facing)
        {
            return Size.GetDimensionXZ(facing);
        }
        ...
        {
            return Position.WorldGrindex.GetDimensionXZ(lib.GetPerpendicularDirection(facing)) + GetLateralWidth(facing) / 2;
        }
        public int GetLongitudinalCenter(Dir4 facing)
        {
            return Position.WorldGrindex.GetDimensionXZ(facing) + GetLongitudinalDepth(facing) / 2;
        }
