...
namespace VikingEngine.LootFest.Map.Terrain.AlgoObjects
{
    ...
    //    public static void Generate(WorldPosition wp, HouseSettings settings, Random seedInstance)
    //    {
    //        bool xdir = lib.RandomBool(seedInstance);
    ...
    //        if (xdir)
    //        {
    //            size = new IntVector3(settings.Length.GetRandom(seedInstance), 1, settings.Width.GetRandom(seedInstance));
    ...
    //        else
    //        {
    //            size = new IntVector3(settings.Width.GetRandom(seedInstance), 1, settings.Length.GetRandom(seedInstance));
    ...
    //        AlgoLib.FillRectangle(floorY, size, (byte)Data.MaterialType.wood_brown);
    //        //clean the surface above the floor
    //        AlgoLib.FillRectangle(wp, size, byte.MinValue);
            
    //        size.Y = settings.Height.GetRandom(seedInstance);
    ...
    //        //wp.Y += size.Y -1;
    //        //size.Y = settings.RoofHeight.GetRandom(seedInstance); //5
    //        ////wp.UpdateWorldGridPos();
    ...
    //                //{
    //                //    AlgoLib.Door(wp, size, dir, DoorSize, windowPositions[i], (byte)settings.DoorMaterial, (byte)Data.MaterialType.dark_red_orange,
