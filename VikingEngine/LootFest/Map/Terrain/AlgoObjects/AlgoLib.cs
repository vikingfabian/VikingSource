    ...
    static class AlgoLib
    {
            ...
            int frameSize, byte framMaterial)
        {
            ...
            if (frameSize > 0)
            {
                ...
                //if (side == Facing4Dir.NORTH || side == Facing4Dir.SOUTH)
                //{
            ...
            int frameSize, byte framMaterial)
        {
           ...
           // new GO.EnvironmentObj.Door(bottomPos, doorSize, material, side, true);
        }
            ...
            byte sideWallMaterial, int sideWallInPush, int sideWallThickness)
        {
            ...
            height = Bound.Max(height, halfWidth);
            ...
            foreach (IntVector2 point in line)
            {
                ...
                for (int l = 0; l < length; l++)
                {
                    ...
                    if (sideWalls)
                    {
                        int wallLenght = lib.SmallestValue(l, length - l - 1);
        ...
        static List<IntVector2> linePassesGridPoints(Vector2 size)
        {
            ...
            int stepLength = Convert.ToInt32(lib.LargestValue(size.X, size.Y));
            float shortStep = 
                (float)lib.SmallestValue(size.X, size.Y) / stepLength;
        ...
        public static void BuildWall(IntVector3 LeftbottomTopPos, IntVector3 size, int thickness, ushort material, HoleSteps2D holeSteps)
        {
            ...
            for (int zpos = 0; zpos < (size.Z + thickness); zpos++)
            {
                foreach (WorldPosition tb in topToBottomSection)
                {
                    ...
                    {
                        wp.SetBlock(material);//LfRef.chunks.Set(wp, material);
                    }
