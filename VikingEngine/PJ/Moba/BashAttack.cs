...
{
    class BashAttack : Graphics.AbsUpdateableImage
    {
        GO.Hero hero;
        Physics.CircleBound bound;
        List<AbsUnit> unitsInRange;

        public BashAttack(GO.Hero hero)
        ...
        {
            Size1D += Ref.DeltaTimeSec * MobaLib.UnitScale * 10f;
            ...
            if (Size1D >= MobaLib.UnitScale * 3.4f)
            {
                DeleteMe();
            }
        }
